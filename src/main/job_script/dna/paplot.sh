...
if $post_analysis_sv_enable ; then
    singularity exec $genomon_img paplot ca \
        $pa_sv_txt \
        $paplot_output_dir \
        CaGMeJ \
...
if $post_analysis_mutation_enable ; then
    if $annovar_enable ; then
        singularity exec $genomon_img paplot mutation \
            $pa_mutation_txt \
            $paplot_output_dir \
            CaGMeJ \
            --config_file $paplot_conf \
...
if $pmsignature_ind_enable ; then
    ...
    do
        singularity exec $genomon_img paplot pmsignature \
...
if $pmsignature_full_enable ; then
    ...
    do
        singularity exec $genomon_img paplot signature\
...
singularity exec $genomon_img paplot index \
    $paplot_output_dir \
    --config_file $paplot_conf \
