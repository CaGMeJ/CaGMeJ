...
if [  -e  $out_dir/ascatngs/${id}.copynumber.caveman.csv ]; then
    ...
    singularity exec $scarhrd_img R -e '
    library("scarHRD");
...
if [ -e $output_dir/facets/$tumor_name/${tumor_name}.out.fit.tsv ]; then
    ...
    sed -e "s/\"//g" $output_dir/facets/$tumor_name/${tumor_name}.out.fit.tsv | awk -F ',' 'NR>4{if( $13 ~ /^[0-9]+$/ && $15 != "NA" && 1 <= $2 && $2 <= 22 ){ print "'$tumor_name'\tchr"$2"\t"$11"\t"$12"\t"$14"\t"$14-$15"\t"$15"\t'$ploidy'"}}' >> $out_dir/scarhrd/${tumor_name}.facets.scarHRD.input.txt
    singularity exec $scarhrd_img R -e '
    library("scarHRD");
